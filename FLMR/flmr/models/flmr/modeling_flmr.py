...
class FLMRModelForRetrieval(FLMRPretrainedModelForRetrieval):
    ...
    def forward(
        ...
        ###########################################################################
        precomputed_scores: Optional[torch.Tensor] = None,  # 사전 계산된 문서 점수
        use_kl_loss: bool = False,  # KL Loss를 사용할지 여부를 나타내는 인자
        kl_loss_weight: Optional[float] = 1.0  # KL Loss의 가중치
        ###########################################################################
        ...
        if use_kl_loss:
            if precomputed_scores is None:
                raise ValueError("KL Loss requires precomputed scores.")

            # colbert-style score 계산
            scores = self.score(Q, D, D_mask)

            # KL Divergence 계산 (log_softmax + softmax)
            kl_loss = F.kl_div(
                F.log_softmax(scores, dim=-1),
                F.softmax(precomputed_scores, dim=-1),
                reduction='batchmean'
            )
            kl_loss *= kl_loss_weight  # KL Loss에 가중치 적용

            # attention/hidden_state 추출
            if output_attentions:
                query_attentions = (
                    query_outputs.text_encoder_attentions,
                ...
                query_attentions = context_attentions = None
            ...
            if output_hidden_states:
                ...
                query_hidden_states = context_hidden_states = None

            return FLMRModelForRetrievalOutput(
                loss=kl_loss,
                kl_loss=kl_loss,
                scores=scores,
                in_batch_negative_loss=None,
                ...
                query_attentions=query_attentions,
